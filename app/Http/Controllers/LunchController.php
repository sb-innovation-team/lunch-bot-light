    ...
    public function signUp (Request $request)
    {
        ...
        if ($lastEater)
        {
            ...
            if ($lastEater->created_at->isToday ())
            {
                ...
                $bot->sendEphemeralMessageToChannel (env ("SLACK_BOT_CHANNEL"), "Je staat al ingeschreven voor deze lunch, <@$user->slack_id>!", $user->slack_id);
        ...
        $bot->sendEphemeralMessageToChannel (env ("SLACK_BOT_CHANNEL"), "Je staat ingeschreven voor de lunch, <@$user->slack_id>, je saldo staat nu op *€$balance->amount*.", $user->slack_id);
